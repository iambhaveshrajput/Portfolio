...
# Resilient import for the specific error you saw
...
class HybridChatbot:
    def __init__(self):
        if not GOOGLE_API_KEY or not PINECONE_API_KEY:
            raise OSError("API Keys missing in Render Environment tab.")
        ...
        try:
            ...
            print(f"⚠️ Pinecone initialization delay: {e}")
    ...
    async def respond(self, question: str, session_id: str = "default"):
        ...
        resp = await asyncio.to_thread(self.llm.invoke, [SystemMessage(content="You are Bhavesh's assistant."), HumanMessage(content=prompt)])
